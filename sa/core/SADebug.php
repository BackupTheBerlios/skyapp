<?php
  /*
   ...
   $Id: SADebug.php,v 1.2 2006/01/26 22:50:25 trinculescu Exp $
  */
...
{    
    ...
    function trace($message, $var, $file, $line, $exit = false) 
    {
        if (ENABLE_DEBUG) {
            $id = md5(microtime());            
            ?>
            <div class="sa_trace_start"><a href="javascript:;" class="sa_trace_start_link" onClick="MM_changeProp('var_<?= $id ?>', '', 'style.visibility', MM_toggleVisibility('var_<?= $id ?>'), 'DIV'); " title="click here to view the output of var_dump(<?= gettype($var) ?>)">:: Trace <?= $message ?> <span style="color: #ff6600"><?= $file ?></span> on line <?= $line ?> ::</a></div>
